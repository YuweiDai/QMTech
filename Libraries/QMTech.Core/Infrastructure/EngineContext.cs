...
namespace QMTech.Core.Infrastructure
{
    ...
    public class EngineContext
    {
        ...
        protected static IEngine CreateEngineInstance(QMTechConfig config)
        {
            if (config != null && !string.IsNullOrEmpty(config.EngineType))
            {
                ...
                if (!typeof(IEngine).IsAssignableFrom(engineType))
                    throw new ConfigurationErrorsException("The type '" + engineType + "' doesn't implement 'QMTech.Core.Infrastructure.IEngine' and cannot be configured in /configuration/QMTech/engine[@engineType] for that purpose.");
                return Activator.CreateInstance(engineType) as IEngine;
            }

            return new QMTechEngine();
        }
        ...
        public static IEngine Initialize(bool forceRecreate, HttpConfiguration httpConfig)
        {
            ...
            {
                var config = ConfigurationManager.GetSection("QMTechConfig") as QMTechConfig;
